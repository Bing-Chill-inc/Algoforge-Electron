...
const PORT = 30000; // Port for the internal server
...
const createWindow = () => {
	...
	win.loadFile("src/algoforge/src/index.html");
};
...
server.get("/AlgoForge.svg", (req, res) => {
...
server.get("/bibliocustom.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/BibliothequeAlgo.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/conditionSortie.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/DictionnaireDonnees.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/erreurs.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/mini/conditionSortie.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/mini/lien.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/mini/pointeur.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/mini/probleme.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/mini/procedure.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/mini/redo.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/mini/structureIterative.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
	const bgColor = req.query.bgColor || "ffffff";
...
server.get("/mini/structureIterativeBornee.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
	const bgColor = req.query.bgColor || "ffffff";
...
server.get("/mini/structureSi.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/mini/structureSwitch.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
...
server.get("/mini/undo.svg", (req, res) => {
	const fgColor = req.query.fgColor || "000000";
		...
		fichiers.forEach((nomFichier) => {
			...
			if (fs.statSync(cheminComplet).isDirectory()) {
				const structureFichier = {
					nom: lireContenuFichier(path.join(cheminComplet, "nom.txt")),
					...
					path: cheminComplet.substring(
						path.join(app.getAppPath(), "src/algoforge/src/Bibliotheque").length + 1
					),
				};
...
server.get("/Bibliotheque/getStructure", (req, res) => {
	// Replace this with the actual path to the root of your library
	const cheminBibliotheque = path.join(app.getAppPath(), "src/algoforge/src/Bibliotheque");
...
server.get("/test", async (req, res) => {
	...
	// await page.goto("https://www.algoforge.fr/render.html");
...
const RENDERPATH = path.join(app.getAppPath(), "src/algoforge/src/render.html");
const RENDERROOT = path.join(app.getAppPath(), "src/algoforge/src/");
server.get("/render", async (req, res) => {
	// Return the render.html file
...
server.get("*", async (req, res) => {
	const paramValue = req.params[0];
	let renderContent = fs.readFileSync(RENDERROOT + paramValue, "utf8");
	console.log(paramValue);
	// Assume the correct content type
	const contentType = paramValue.endsWith(".html")
		? "text/html"
		: paramValue.endsWith(".css")
		? "text/css"
		: paramValue.endsWith(".js")
		? "text/javascript"
		: paramValue.endsWith(".json")
		? "application/json"
		: paramValue.endsWith(".png")
		? "image/png"
		: paramValue.endsWith(".jpg")
		? "image/jpg"
		: paramValue.endsWith(".svg")
		? "image/svg+xml"
		: paramValue.endsWith(".php")
		? "image/svg+xml"
		: "text/plain";

	if (paramValue.endsWith(".php")) {
		// Remplacer les variables PHP
		// À de nombreux endroit, nous aurons <?php echo $_GET['fgColor'] ?>, qu'il faudra remplacer par req.query.fgColor
		renderContent = renderContent.replace(/<\?php echo \$_GET\['(.*?)'\] \?>/g, (match, p1) => {
			return req.query[p1];
		});
	}
...
server.post("/png", async (req, res) => {
	...
	// Set the page content to the provided HTML
	await page.goto(`http://localhost:30000/render`);

	await page.evaluate((contenu) => {
		espacePrincipal.chargerDepuisJSON(JSON.parse(contenu), false, false);
	}, htmlContent);
...
server.post("/jpg", async (req, res) => {
	...
	// Set the page content to the provided HTML
	await page.goto("http://localhost:80/algoforge/src/render.html");
